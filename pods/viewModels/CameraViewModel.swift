...
//          }
//      }
    func addVideoItem(podId: Int, email: String, label: String, completion: @escaping (Bool, String?) -> Void) {
        guard let previewURL = previewURL else {
            completion(false, "No preview URL available.")
            return
        }
        ...
        if self.isWaveformEnabled {
            ...
            self.transcribeAudioUsingBackend(from: previewURL) { transcribedText in
                DispatchQueue.main.async {
                    let metadata = transcribedText?.replacingOccurrences(of: "stop recording", with: "", options: .caseInsensitive) ?? label
                    print("Completing video addition with metadata: \(metadata)")
                    handleCompletion(metadata)
                    if self.isWaveformEnabled {
                        self.isWaveformEnabled = false
            ...
            print("Waveform not enabled, using provided label.")
            handleCompletion(label)
        }
    }

    func addPhotoItem(podId: Int, email: String, label: String, completion: @escaping (Bool, String?) -> Void) {
        guard let selectedImage = selectedImage else {
            completion(false, "No image selected.")
            return
        }

        NetworkManager().addNewItem(podId: podId, itemType: "image", videoURL: nil, image: selectedImage, label: label, thumbnail: selectedImage, email: email) { success, message in
            if success {
                print("Photo item added to pod successfully.")
            } else {
                print("Failed to add photo item to pod: \(message ?? "Unknown error")")
