    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                // If no search results, show combinedLogs
                    ...
                    VStack(spacing: 0) {
                        ForEach(foodManager.combinedLogs.indices, id: \.self) { index in
                            ...
                            if index < foodManager.combinedLogs.count - 1 {
                                Divider()
                                    .padding(.leading, 16) // CHANGED: add leading padding
                            }
                        }
                    }
...
private struct MealListView: View {
    ...
    var body: some View {
        // CHANGED: apply .listRowSeparator(.hidden) once on the entire list,
        // and add a manual Divider() after each meal row in the ForEach.
        List {
            Section {
                CreateMealButton(path: $path)
                    // remove the old .listRowSeparator(.hidden) here
                ...
                Text("History")
                    ...
                    .fontWeight(.bold)
                    .padding(.top, 8)
                    // remove the old .listRowSeparator(.hidden)
                ...
                ForEach(foodManager.meals.indices, id: \.self) { index in
                    ...
                    // remove the old .listRowSeparator(.hidden) from here
                    .listRowInsets(EdgeInsets(top: 4, leading: 0, bottom: 4, trailing: 0))
                    
                    // CHANGED: now we add a single divider after each meal row, except last
                    if index < foodManager.meals.count - 1 {
                        Divider()
                            .padding(.leading, 16) // Leading padding to match styling
                    }
                }
            }
        }
        .listStyle(.plain)
        .listRowSeparator(.hidden) // CHANGED: hide system row separators for the entire List
        .safeAreaInset(edge: .bottom) {
...
//                 // If no search results, show combinedLogs
//                 if searchResults.isEmpty && !isSearching {
//                     // The "cards" that show your recent logs
//                     VStack(spacing: 0) {
//                         ForEach(foodManager.combinedLogs.indices, id: \.self) { index in
//                             let log = foodManager.combinedLogs[index]
                            
//                             // Render appropriate content based on log type
//                             switch log.type {
//                             case .food:
//                                 if let food = log.food {
//                                     FoodRow(
...
//                                 }
//                             case .recipe:
//                                 // If needed, display CombinedLogRecipeRow 
//                                 // (the code is similar but omitted here for brevity).
//                                 EmptyView()
//                             }
...
//             var lastAddedFood: Food? = nil
//             for mealItem in fullMeal.mealItems {
//                 let servingsValue = Double(mealItem.servings.trimmingCharacters(in: .whitespacesAndNewlines)) ?? 1.0
//                 let servingText: String
//                 if let text = mealItem.servingText, !text.isEmpty {
//                     servingText = text
//                 } else {
//                     if let unit = mealItem.servings
//                         .components(separatedBy: CharacterSet.decimalDigits.union(CharacterSet(charactersIn: ".")))
//                         .last, !unit.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
//                         servingText = unit.trimmingCharacters(in: .whitespacesAndNewlines)
//                     } else {
//                         servingText = "serving"
//                     }
...
//     private var displayCalories: Double {
//         // If meal.calories is set, use that, otherwise compute from items/macros
//         if meal.calories > 0 {
//             return meal.calories
//         }
//         if !meal.mealItems.isEmpty {
//             let total = meal.mealItems.reduce(0) { $0 + $1.calories }
