...
struct DuplicateFoodSheet: View {
    let food: Food
    ...
    // MARK: - Initializer
    init(food: Food, onFoodCreated: ((Food) -> Void)? = nil) {
        self.food = food
        self.onFoodCreated = onFoodCreated

        // Pre-populate basic info with "Copy of" prefix
        _name = State(initialValue: "Copy of \(food.displayName)")
        _brand = State(initialValue: food.brandName ?? "")
        _basedOn = State(initialValue: .serving)

        // For weight, prefer servingWeightGrams, then gramWeight from measures, finally servingSize
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Duplicate Food")
    ...
    private func submitFood(action: CreateFoodAction) {
        ...
        let newFood = buildFood()

        foodManager.createManualFood(food: newFood, showPreview: false) { result in
            isSubmitting = false
            ...
            case .failure(let error):
                print("Failed to create duplicate food: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func buildFood() -> Food {
        ...
        // Create a NEW food with a new ID (duplicate, not update)
        return Food(
