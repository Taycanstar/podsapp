...
struct FoodChatFood: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case foodNutrients, servingSizeText, servingSize, servingSizeUnit, numberOfServings  // .convertFromSnakeCase handles snake_case
    }
}
...
class NetworkManager {
    ...
    /// Get a signed promotional offer from the backend
    /// - Parameters:
    ///   - productId: The product identifier (e.g., "humuli_pro_yearly")
    ///   - offerId: The promotional offer ID (e.g., "exit_offer_yearly_90off")
    ///   - userEmail: The user's email address
    /// - Returns: Dictionary with signature, nonce, timestamp, and key_id
    func getPromotionalOfferSignature(productId: String, offerId: String, userEmail: String) async throws -> [String: Any] {
        guard let url = URL(string: "\(baseUrl)/promotional-offer-signature/") else {
            throw NetworkError.invalidURL
        }
        ...
        if httpResponse.statusCode == 200 {
            ...
            throw NetworkError.serverError("Failed to get promotional offer signature: \(errorMessage)")
        }
    }
    ...
    func streamAIResponse(
        ...
        guard let url = URL(string: "\(baseUrl)/ai-stream/") else {
        ...
        guard let body = try? JSONSerialization.data(withJSONObject: payload) else {
            onError(NetworkError.jsonEncodingFailed)
            return nil
        }
        request.httpBody = body
        ...
        let delegate = StreamingDelegate(
            ...
            onComplete: { [weak self] in
                self?.streamingSessions[token] = nil
                onComplete()
            },
        ...
        let session = URLSession(configuration: .default, delegate: delegate, delegateQueue: nil)
        streamingSessions[token] = session
        let task = session.dataTask(with: request)
        task.resume()
        return token
    }
    ...
    func cancelStream(token: UUID) {
        if let session = streamingSessions.removeValue(forKey: token) {
